...
/**
 * Tabele gildii (kształt 1:1 z src/api/v1/guildApi.ts row-shapes + scripts/
 * guild_migration.sql na Supabase). Osiem tabel: `guilds`, `guild_members`,
 * `guild_join_requests`, `guild_boss_state`, `guild_boss_attempts`,
 * `guild_boss_contributions`, `guild_treasury_items`, `guild_treasury_logs`.
 *
 * IDEMPOTENTNA: każda tabela ma własny guard hasTable → na realnej Supabase
 * (gdzie tabele już istnieją) up() jest no-opem. Służy testom (sqlite in-memory)
 * i jako autorytatywna dokumentacja schematu. down() = no-op, żeby
 * migrate:rollback nie ruszył realnych tabel.
 *
 * BIGINT tam, gdzie wartości przekraczają int32: boss HP (tier 50 ≈ 1.4e11),
 * XP gildii (suma zadanych HP), damage — wszystko po stronie „1 HP = 1 XP".
 */
    ...
    public function up(): void
    {
        ...
        if (! Schema::hasTable('guild_boss_state')) {
            Schema::create('guild_boss_state', function (Blueprint $table): void {
                $table->uuid('id')->primary();
                $table->uuid('guild_id')->index();
                $table->string('week_start');              // YYYY-MM-DD (poniedziałek UTC)
                $table->integer('boss_tier')->default(1);
        ...
        if (! Schema::hasTable('guild_boss_attempts')) {
            Schema::create('guild_boss_attempts', function (Blueprint $table): void {
                ...
                $table->uuid('character_id')->index();
                $table->string('character_name');
                $table->string('attempt_date');            // YYYY-MM-DD
        ...
        if (! Schema::hasTable('guild_treasury_items')) {
            Schema::create('guild_treasury_items', function (Blueprint $table): void {
                $table->uuid('id')->primary();
                $table->uuid('guild_id')->index();
                $table->text('item_data');                 // serializowany snapshot itemu
